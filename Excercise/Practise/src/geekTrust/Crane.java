...
//		
		...
		System.out.println(canReach(new int[] {1,3},new int[] {2,6},1,5));
		
		System.out.println(canReach(new int[] {5,4,1},new int[] {5,4,6},4,8));
	...
	public static boolean canReach(int[] a, int[] p, int b, int e) {
		...
		Collections.sort(pairs, (x,y) -> x.x == y.x ? x.y - y.y : x.x - y.x);
...
//		return false;
	...
	public static boolean found(int i, int b, int e, List<Pair> pairs, int in, int n) {
		...
		if(in == 1 && pairs.get(i).x > b) {//if it's not overlap
			return false;
		}
		if(in == 0 && pairs.get(i).y < b) {//if it's not overlap
			return false;
		}
		if(pairs.get(i).x <= e && pairs.get(i).y >= e) {
			return true;
